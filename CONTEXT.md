...
- **工具（tool）**：画布交互模式（选择/描绘）。单一真相在 toolStore（`tool: 'select' | 'trace'`），取色期间不变。
- **取色（color pick）**：挂起当前工具的系统模态（macOS NSColorSampler / Windows 截屏+覆盖层）。`pickSession: { temporary } | null` 承载，非工具值；退出仅清 session，工具原样返回。
...
- **撤销历史（history）**：撤销/重做双栈私有化进的泛型深 module（`src/store/history.ts`，#50），移出 Zustand state（非反应式，无 canUndo/canRedo 订阅）。interface `createHistory<T>({ snapshot }) → { record, undo, redo, clear }`：`record(prev, hint?)` 押「编辑前」快照（内部 snapshot(prev)）、`undo/redo(cur)` 对称出栈恢复（cur 经 snapshot 押入对侧栈）、`clear()` 清空双栈 + 复位 lastKey（openProject / createProject / 测试重置）；双栈各上限 100 条、超限驱逐最旧（栈底丢弃）；快照策略注入（project 域注入 `snapshotProject`，结构浅拷贝容器、path/dataUrl 大字符串按引用共享）。coalesce：调用方显式传 `'<property>:<elementId>:<gestureId>'`，同手势连续提交合并为一条撤销记录（PropertyPanel 滑杆 pointerdown/keydown 生成自增 gestureId，键盘方向键连续改值可合并）；transform 回灌 / addPaper / reorder / 底图开关不传 hint，离散动作永不合并；废除时间窗、无时钟依赖。undo/redo/clear 复位内部 lastKey——防 undo 后继续拖同一滑杆把新提交错误并链。实例随 store 单例共生（`create()` 回调内 new），不做模块级单例（防跨测试污染）。no-op 识别留 store 侧（commitEdit 引用比较 + diffProject，先于 history.record）。
- **项目持久化（projectPersistence）**：保存链路收拢 module（`src/io/projectPersistence.ts`，#60）。interface `createProjectPersistence(deps) → { save, open, schedule, dispose }`：`save()` 即原 flush 语义（手动保存无视防抖、首存仍弹位置）、`open()` 编排「pickOpenPath → readProjectFile → store.open」且 reject 传播、`schedule()` 改动防抖写盘入口、`dispose()` 取消未到期写盘。deps 两个窄 adapter 由 App 组装时投影——store 投影 useProjectStore 导出（getProject / getSavePath / setSavePath / setSaveStatus / open），file 投影 io/projectFile 导出（pickSavePath / pickOpenPath / writeProjectFile / readProjectFile）；编排逻辑可 headless 直测、不依赖 Tauri 运行时。内部保留 `createAutosaveController` 独立实例（防抖 / 首存弹位置 / in-flight 补写 / flush 语义不变，不重写 autosave 内部）。subscribe 不收进 module（#52 Q4：避免依赖 store 订阅形状、破坏 adapter 抽象），壳保留一行订阅 → schedule；打开后 store 替换 project → 订阅触发 schedule → 500ms 后一次无害写回（内容=磁盘内容，顺带把打开前 saveStatus='error' 刷新为 saved）属预期行为（#52 Q5 / T12）。
